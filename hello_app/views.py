...
def master_data():
    if request.method == 'POST':
        ...
        if master_search:
            master_search = [dict(zip([column[0] for column in cursor.description], row)) for row in master_search]
            return jsonify({"data": master_search})
        else:
            flash("No equipment found with this MID!", "danger")  # show pop-up alert
...
def master_record(master_id):
    if request.method == "POST":
        ...
        if master_results:  # show master results
            master_results = [dict(
                zip([column[0] for column in cursor.description], row)) for row in master_results]
            cursor.execute(
